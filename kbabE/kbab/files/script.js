...
links.forEach(function(link) {
  link.onclick = function(e) {
    ...
    if (h4) {
      ...
      e.preventDefault(); // يمنع الانتقال عند الضغط
...
function updateModalPriceDisplay() {
  ...
  if (selectedType === "حار") extra += 3;

  const finalPrice = (basePrice + extra) * quantity;
  Rr.textContent = `${finalPrice} جنيه`;
}

// عند الضغط على زر الحجم
...
openButtons.forEach((btn) => {
  btn.addEventListener("click", () => {
    if (window.innerWidth <= 940) {
      ...
      document.querySelectorAll(".type-btn").forEach(b => b.classList.remove("selected"));
      document.querySelector('[data-type="عادي"]').classList.add("selected");
...
sendToCartBtn.addEventListener("click", () => {
  ...
  // تحديث عدد العناصر في أي عنصر في الصفحة
...
no.forEach(function(no) {
  no.onclick = function(ev) {
    ...
    ev.preventDefault(); // يمنع الانتقال عند الضغط
  };
});
...
opp.forEach(function(opp) {
  opp.onclick = function(eve) {
     eve.preventDefault(); // يمنع الانتقال عند الضغط
  };
});
...
function updateModalPriceDisplaye() {
  ...
  if (selectedTypee === "حار") extraa += 3;

  const finalPrice = (basePricee + extraa) * quantityy;
  priceDisplay.textContent = `${finalPrice} جنيه`;
}
